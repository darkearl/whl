...
require([
    ...
    function _hasRequiredInFields(fields) {
        var result = false;
        $.each(fields, function(name) {
            result = result || _.some(fields[name], function(item) { return !item.optional; });
        });
        return result;
    }

    function _hasDefaultInFields(fields) {
        var result = false;
        $.each(fields, function(name) {
            result = result || _.some(fields[name], function(item) { return item.defaultValue; });
        });
        return result;
    }
    ...
    function changeVersionCompareTo(e) {
        ...
        if (compareVersion && articleVersions[group][name][0] === selectedVersion || version === selectedVersion) {
            ...
            if (entry.parameter && entry.parameter.fields) {
                ...
                fields._hasRequiredInParameterFields = _hasRequiredInFields(entry.parameter.fields);
                fields._hasDefaultInParameterFields = _hasDefaultInFields(entry.parameter.fields);
            }

            if (entry.error && entry.error.fields) {
                fields._hasTypeInErrorFields = _hasTypeInFields(entry.error.fields);
                fields._hasRequiredInErrorFields = _hasRequiredInFields(entry.error.fields);
                fields._hasDefaultInErrorFields = _hasDefaultInFields(entry.error.fields);
            }

            if (entry.success && entry.success.fields) {
                fields._hasTypeInSuccessFields = _hasTypeInFields(entry.success.fields);
                fields._hasRequiredInSuccessFields = _hasRequiredInFields(entry.success.fields);
                fields._hasDefaultInSuccessFields = _hasDefaultInFields(entry.success.fields);
            }

            if (entry.info && entry.info.fields) {
                fields._hasTypeInInfoFields = _hasTypeInFields(entry.info.fields);
                fields._hasRequiredInInfoFields = _hasRequiredInFields(entry.info.fields);
                fields._hasDefaultInInfoFields = _hasDefaultInFields(entry.info.fields);
            }
    ...
    function addArticleSettings(fields, entry) {
        ...
        if (entry.header && entry.header.fields) {
            ...
            fields._hasRequiredInHeaderFields = _hasRequiredInFields(entry.header.fields);
            fields._hasDefaultInHeaderFields = _hasDefaultInFields(entry.header.fields);
        }
